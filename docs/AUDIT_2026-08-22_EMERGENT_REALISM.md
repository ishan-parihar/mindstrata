...
**Tally progression:** Iter-239: 287/17 → Iter-240 (faction lifecycle): 290/14 →
Iter-241 (belief charging): 293/11 → **Iter-242 (pairing pipeline + health sync): 298/6.**

Healed along the way: the four faction tests, moral-panic cluster (3), marriage mechanics,
kinship-tie formation, the conception→pregnancy→birth pipeline, the fertility-multiplier
liveness, taboo-shame cascade, and the regime-change path.

Remaining six are single-system calibration drift, each an independent iteration:
`collapse_famine_timing_shapes_plague_mortality`, `institutional_rank_weighted_into_effective_status`,
`memory_system_produces_plan_taxonomy_and_trace_properties`, `motivation_emotional_context_is_live`,
`neural_like_prediction_error_folds_are_live_and_directional`, `noospheric_belief_confidence_sustains_conviction`.
