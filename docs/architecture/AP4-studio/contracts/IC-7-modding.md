...
* `self.lore_archetypes` (per-agent, derived deterministically from `archetype_for_claim` in `crates/mindstrata-development/src/lore.rs:100`, moved from `psychology/lore.rs` in the crate ladder) — per-tick emergent, read-only (DC-2.7 @61e78b1, surfaced in dossier @c032ae3)
