...
| 2 | §8.1.4 | **PARTIAL (Iter 116)** — three channels live: loneliness→social-seeking (Iter 98, `interact_chance` + loneliness×0.3), tenderness→helping (Iter 99, `help_propensity` + tenderness×0.5), humiliation→failed-threat escalation amplifier (Iter 116, `should_escalate` × (1 + humiliation×0.3)); Iter 116 also added the per-tick proportional decay (0.12/tick) breaking the write-only saturation. Still unwired: contempt, awe, envy, relief, despair, nostalgia, moral_outrage, disgust, jealousy | Iter 116 | Partially wired |
