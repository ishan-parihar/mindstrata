...
| 2 | §8.1.4 | **PARTIAL (Iter 122)** — five channels live: loneliness→social-seeking (Iter 98, `interact_chance` + loneliness×0.3), tenderness→helping (Iter 99, `help_propensity` + tenderness×0.5), humiliation→failed-threat escalation amplifier (Iter 116, `should_escalate` × (1 + humiliation×0.3)), contempt→failed-threat escalation amplifier (Iter 122, `should_escalate` × (1 + contempt×0.3); ONE-SIDED identity-at-zero — probe-pinned never produced in calibrated windows → zero-blast), despair→failed-threat escalation pacifier (Iter 122, `should_escalate` × (1 − despair×0.5) floored at 0.5; ONE-SIDED identity-at-zero → zero-blast). Iter 116 also added the per-tick proportional decay (0.12/tick) breaking the write-only saturation. Still unwired: awe, envy, relief, nostalgia, moral_outrage, disgust, jealousy (awe/relief/nostalgia are LIVE producers — a consumer on them would be a calibrated change, not zero-blast; envy is zero but its natural consumer is the relationship/patronage layer, not failed-threat escalation) | Iter 122 | Partially wired |
