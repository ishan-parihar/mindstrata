...
| 10 | §10.2 / §11.2 | **WIRED (Iter 102)** — directed `power_balance` fed into `should_escalate` as a multiplicative dominance scale (0.5× subordinate → 1.5× dominant, identity at zero → legacy byte-identical); the violence-escalation decision now reads the full relational-power stack. Remaining: the three relational fields (sensory/social/noospheric) still have no consumer; `institutional_rank` still not weighted into `effective_status()` | Iter 39/81/55 | Partially wired |
