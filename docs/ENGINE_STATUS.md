...
gene-pinned-0.5 control reproduced the then-stored golden `metric_hash` `0xb2a1be3b18fbb46d`
byte for byte (that hash is historical — i399 regenerated the riverford baseline to
`0x026c07db35273eda`; the control reproduced whatever was stored at the time, which is the
property being claimed); the natural-gene divergence began at tick 657 for agent 0 alone), but the surface's
...
1. **v1→v2 relationship migration (charter DECIDED: finish it subsystem by subsystem)** — i353 measured per-pair divergence (90% of pairs >0.01 at N=48/20K). **i369 landed the first migration:** the comfort/soothing path now reads the dyadic v2 store (three-way runnable pin; collapse golden one birth flipped 13→12, in-contract). Remaining readers queued: norms_impl, household, births_deaths; marriage pass REFUTED (i353); the cognitive mean-reversion writer landed (i376). **i384 migrated the economy trade pair (read + write)** — the last self-contained v1 pair — after its probe measured the divergence at 6–40× the population average on exactly the pairs that trade (v1 0.9174/0.8068/0.7892 vs v2 0.8242/0.7036/0.6551) and a 2.8–4.0% price error on the busiest dyads; post-migration the read-source delta is 0.34–0.52%, traded-pair v2 trust sits at 0.93–0.98, and `trade_price_reads_the_dyadic_store_and_writes_it` guards the store choice. The remaining v1 writers' gain schedule is the next migration commit, and **its probe has landed (i393, `i393_speech_act_store.md`) — the item's shape was corrected twice by measurement.** The divergence is a **mid-day** phenomenon: the stores agree to 0.0009–0.0039 on a daily boundary (i376's sync works, for `trust`) and rebuild to **0.169 mean / 0.999 max 72 ticks later**, so a v1 trust reader's verdict depends on the tick it samples — while **`affection` is never synced at all** (0.187 mean, max 1.000 at 50K, unbounded in horizon), so some v1 affection readers are reading a store with no inbound path. The three-regime projection rejects importing the v1 gain (it drives dyadic saturation 50% → 61% at 50K — the condition i376 diagnosed as v1's failure) in favour of **delete the gain, keep the dyadic schedule**. Census: **`Relationship.kind` has no production reader** (a write-only §19.5.G ladder — retire it rather than migrate it, since `RelationshipV2.stage` is the live replacement), and **deleting the interaction gain does not make v1 derived** (`legal_impl` −0.2 and `marriage` +0.2/+0.3 keep writing it, above the deleted gain's magnitude, so they are separate behavioural moves). **The inert half has landed**: the write-only §19.5.G `RelationshipKind` ladder is retired (`evolve_relationship_kind` deleted; `Relationship.kind` frozen at its construction value), proven zero-blast by both goldens staying byte-identical — which is the reader census's "no production reader" claim verified rather than asserted. **The migration itself is order-constrained, and now measured**: deleting the v1 writer *first* gave 299/15/1 in 656 s (liveness producers dark — §2.3 forbids re-pinning them), while moving only the two load-bearing folds gave 301/13/1 in 143 s with four pins of four different kinds (a `contagion_delta` Fixed-4 truncation the move exposed, a kin-labelling correctness break, a `peer_status` band edge, a §4.13 manipulation failure). So the readers migrate first (norms_impl, household, births_deaths, marriage/attachment), then the v1 application goes **with `bonding_rate`/`conflict_escalation_rate` re-hosted on the dyadic magnitude** (`Fixed::ONE` defaults, so the re-host is value-neutral and keeps their liveness pins honest), then the folds — full map in `evidence/i393_speech_act_store.md` §6. **First reader landed (i398, `evidence/i398_norms_store.md`):** the `norms_impl` belief-evidence channel now reads the dyadic store — provably inert (both goldens byte-identical, 314/0/1), because the stores agree at every pinned horizon (the probe's early-window 0.082 divergence is the pre-sync-boundary spike, gone by 20K), the consumer is linear toward an equilibrium, and evidence is symmetric. The `norms_impl` **writers were moved and reverted with the mechanism understood**: the v1 damage write was transient (i376's daily sync erased it within a day); on v2 the same −0.3 is persistent, which re-arms the Iter-185 low-trust→threat spiral its 0.12 rate was calibrated against — measured: one extra death by 10K, `peer_status` 0.2255 (floor 0.3), fear-contagion presence 9/12, a conception pinned tick slid. The write goes back to v1 with the finding recorded at the site; the magnitude gets re-sized **for persistence** by a violence-family sweep before the move. **First behavioural landing (i399, `evidence/i399_marriage_store.md`):** `marriage.rs` was not a reader but a **closed loop inside the ghost store** — the formation gate reads v1 affection/trust, the bond boost (+0.2/+0.3, both directions) writes v1, and the jealousy pass reads v1 trust, while v1 `affection` has no other production reader and no decay. Probe leg B: married-pair end affection **v1 0.937 (pinned flat, no decay) vs v2 0.878 (decayed, differentiated)** — the bond was invisible to the machinery built to model it. Read+write migrated together (i384 rule). Consequences, all measured: **7 snapshots + both goldens re-baselined** with the mechanism's direction (trust 0.6345 → 0.6475, quality 0.4124 → 0.4217, stage distribution deepens to `Confidant`, 10K stress 0.3605 → 0.3679 i.e. rising, not saturating; collapse `agent_count` 12 → 13, one more crisis survivor); the **revolution family re-anchored after its 10-seed sweep proved the producer not starved** (total **15 → 18**, 3/10 → 5/10 firing seeds, members {12345, 7, 11}); and a **stale fixture hung the suite for >300 s** — `marriage_forges_spouse_and_inlaw_kinship` pinned only v1, so nothing suppressed the other pairs and agent 0 paired with agent 1 (the 900-step window ran to exhaustion). **Migration lesson, now doctrine-shaped: a fixture that pins only the store the pass no longer reads fails as a HANG, not a diff.** **The next readers were measured and REVERTED (i400, `evidence/i400_cluster_trust.md`):** `social_cluster.rs`'s §8.1.9 ToM pair trust and §19.5.I knowledge-diffusion `source_trust` read v1 today, and the probe gave a clean shape — zero fallback misses on either store, mean |v1−v2| 0.0096/0.0195 (N=12/48) — but an unbounded tail (max 0.70/1.00) sitting on a live band edge (`infer_intent` is Friendly only above trust 0.5; 0.8–1.8% of sampled pairs straddle it). The suite settled it: **304/10/1 in 211 s** against **314/0/1 in 165 s** reverted, the decisive failures being two **deliberate dormancy contracts** ("the lifecycle must stay dormant within the 500-tick window"; "no pregnancy may exist in the golden window"), i.e. a design state that §4.1 forbids widening for a ~0.01-mean accuracy change on a soft consumer. **Revised sequence: with a band edge involved, the remaining v1 trust readers and the interaction-gain deletion move in ONE commit**, so the dormancy windows and the acceptance floor are re-derived once against the whole new trust surface.
