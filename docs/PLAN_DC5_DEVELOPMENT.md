...
| Dead genome fields | **three remain unwired**: `sensory_acuity`, `aggression_threshold` (unhandled), and `novelty_seeking` (measured-and-rejected — kept inert deliberately until a linear consumer exists). Closed: `puberty_age` (row 1), `chronic_pain_risk` (row 3) | `scripts/field_census.py`; `ENGINE_STATUS.md` §6 |
...
**First arc status: i387 → i388 → i389 → i390 → i391 → i392 (rows 1 and 3) DONE, row 2 rejected,
and its precondition i392b DONE; i392's remaining rows are `sensory_acuity` (row 4) and
`aggression_threshold` (row 5), then the next act item is i393 (the speech-act store).** i392's row 2 (`novelty_seeking` → the exploration driver) was
