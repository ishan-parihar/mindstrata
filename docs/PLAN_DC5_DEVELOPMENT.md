...
| **i403** | The writer deletion itself — now carrying i402's folded read move: `bonding_rate`/`conflict_escalation_rate` re-hosted on the dyadic magnitude (**re-host target note**: the dyadic per-act write already exists — pass_social's speech-act block applies `record_positive/record_negative` per interaction, trust +0.02×vol×mag / affection +0.015×vol×mag at the i393/i384 calibration; the re-host SCALES those magnitudes by the rates (both identity-1.0 → value-neutral), it does not add a second write channel), `affection` added to the i376 daily sync, `social_reciprocal_factor` retired, the Fixed-4 `contagion_delta` truncation fixed first (the product `perceived_stress × 0.05` quantizes below the 5e-5 floor for stress < ~0.002, stripping the whole low range; the f64-shadow fix keeps the `(stressed × rate).clamp_01()` contract and its identity-at-zero/monotone unit test intact — a pure one-line numeric repair with no dependency on the deletion, so it opens the commit); THEN re-run the i402 probe against the post-deletion affection surface and land the kind-schedule read move + the redesigned gate (relative/cohort candidate) in the same commit | one commit, all of (b)(c)(d) from i401 §6's corrected sequence **plus** i402's folded read move + gate redesign; i402 probe re-run = the pre-change leg | every pinned consumer accounted for; no producer darkened (§4.3); the gate re-derived against the *new* surface, never the saturated one |
