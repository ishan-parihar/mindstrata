...
|---|---|---|---|
| **i387** [M] | The utility ledger has never been decomposed: i380 proved `Socialize` loses the argmax and that two single-knob repairs fail, but not **which term** decides the winner | extend `i346_decision_census` with a per-candidate, per-term decomposition (goal-alignment / dominant-need urgency / identity affinity / norm / somatic / institution / routine override) over ≥100K arbitrations | the winner's deciding term is named and `Socialize`'s gap quantified → **disposition** (live-or-delete) recorded |
| **i388** | Either revive `needs.social` with the channel its siblings have (per the i387 ledger) **or** delete the inert term and re-contract sociality as routine-carried — decided by measurement, not taste | `i388_socialize_channel` re-run of the census after the change | Socialize's utility share is either ≥1% of arbitrations, or the term is deleted and the deletion pinned |
