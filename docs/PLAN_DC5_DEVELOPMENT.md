...
| **i428** | **DONE — LANDED TRIMMED** (`evidence/i428_writer_deletion_trimmed.md`, 2026-09-25) — the i403 arc re-assembled minus its two measured-poison read moves (kind-schedule + social_cluster ToM/§19.5.I restored to v1 after i427's event-stream census showed the bars already 89–100% open on v2 and i401 measured the social_cluster reads alone killing conception). Kept: the v1 interaction-writer deletion, rate re-host, credibility move, memory_ops fold, affection sync leg, degree re-point (§4.17 companion), witness on v1. **Crisis liveness restored: 17 revolutions on 6-of-10 seeds (post-arc baseline: 2 total), panics back on the pestilence corpora (4 seeds at 5–41 events), `moral_panic_lifecycle` green.** Dispositions all probe-evidenced: both goldens custody-re-baselined (collapse agent_count 13→12), 7 snapshots accepted after diff review, convictions re-contracted to the ratio form (1.548×, dead-producer floor 0.15 HIGH-leg), attachment to a majority floor (family {56.2, 41.7, 70.8}%), peer_status floor 0.25 with the non-isolated mechanism honestly named, fear-contagion scoped to the i425 quantization floor, revolution family re-discovered both-legs as **{99, 42, 11}** (recorded finding: seeds 23/5 fire coups without Elder succession). Suite **297/14 → 311/0/1** (719 s) | `i428_pin_reanchor_sweep` + `i428_revolution_leg_sweep` + `i399_revolution_sweep` + `i381_blast_radius` + `i276_golden_recompute` | §4.1 W1 arc CLOSED: the v1 interaction write is gone; the full arc (`aa423ee0`, pinned as `stash@{0}` with a do-not-reapply name) is superseded for wholesale application and retained as source for the remaining v1 sites (`legal_impl`/`marriage` writers, witness) which are separate iterations |
| **i426** | **DONE — MEASURED VERDICT** (`evidence/i426_belief_formation_surface.md`, 2026-09-25) — i403 §6 precondition 1 ANSWERED: the honest-surface deficit is **formation-time** with a smaller steady-state residual. Per-bucket gate-clearing shares on both stores (mirrored gate expressions; sampled 144 intra-day ticks post-sync) over pestilence {5,42,12345}@20K: the founding belief ecology forms entirely in [0,5K) (36–39 of 36–45 distinct beliefs first-seen there) while v2's diffusion-acceptance clearing ran 48.9–67.1% vs v1's 94.4–97.1%; by 18K v2 reaches 81–94% vs v1's 97–99%. Two structural facts measured: (a) v1 is a DENSE spawn matrix (`trust = U(0.3,0.7)` per ordered pair, population.rs:774) — half the graph clears `>0.5` by founder lottery before any interaction, while v2 starts at its stranger prior and earns; (b) `cultural.openness ≡ 0.500` unwired — diffusion acceptance ≡ ToM `trust > 0.5` in production (dead-field row recorded, separate iteration). Panic charge law refuted as a trust channel (distress-driven daily fold; near-universal holder coverage). Consequence: the `>0.5` bars express "half the spawn draw" on v1 and "earned past the prior" on v2 — the re-derivation must be distributional (percentile/baseline form, i381 pattern) sized by the gap method, and each channel's event-stream input must be measured at its own scope first | `i426_belief_formation_surface` (contacted-pair gate shares per surface per bucket; belief stock + first-seen cohorts; openness census) | precondition 1 satisfied: re-derivate the gates distributional before re-applying `stash@{1}` |
