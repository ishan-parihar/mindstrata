...
| **i402 (LIVE)** | **The interaction-kind schedule reads the dyadic store** — `choose_interaction`'s trust/affection pair is the last high-traffic v1 reader, and i401 §3 proved its calibration rode the ratchet | `i402_kind_schedule`: a **kind-mix probe** (Comfort / Gossip / Help / Teach / Talk shares at N=12/48 over 20K, both stores) before touching; then the read move **plus** a re-derivation of the two gating thresholds (0.2 / 0.7) against the new affection surface in one commit | the kind mix is measured before/after with the mechanism line; Comfort/Gossip shares move in the predicted direction; attachment liveness returns to a *derived* band (not a widened one) |
| **i403 (LIVE, after i402)** | The writer deletion itself — now with its blast radius known (`bonding_rate`/`conflict_escalation_rate` re-hosted on the dyadic magnitude, `affection` added to the i376 daily sync, `social_reciprocal_factor` retired, the Fixed-4 `contagion_delta` truncation fixed first) | one commit, all of (b)(c)(d) from i401 §6's corrected sequence | every pinned consumer accounted for; no producer darkened (§4.3) |
...
| **i402** | **The interaction-kind schedule reads the dyadic store** — `choose_interaction`'s v1 trust/affection pair gates the interaction *kind*, which is the attachment channel's producer; migrate it with a kind-mix probe and re-derive the 0.2/0.7 gates in the same commit | W1 | i401 §3 (the ratchet calibration) |
...
| **i421** | **DONE** (`evidence/i421_citation_gate.md`, 2026-09-24) — doc_index check 4: every `*.md\|rs\|py\|sh` token cited in an AUTHORITY/ACTIVE doc must resolve through the project's path-convention ladder (repo/docs/doc-parent/AP4-shorthand/crate-relative/bare-basename); HISTORICAL/REFERENCE docs exempt — frozen records, the same scope rule freshness uses; 23 curated historical/vendor cites (i386's ghosts and moved files, vendor `realms.md`/`rays.md`, C2's `tests/comparison.rs` deletion) each with its mechanism in-script. Found+fixed **2 real drift sites** (AGENTS §7's pre-split examples `sim/family.rs` → `sim/household.rs`, `sim/tests.rs` → `sim/tests/mod.rs`) and 1 extractor false-positive class (`cardiovascular.shock_risk` ≠ `cardiovascular.sh`). **Proven to trip**: planted ghost → exit 1 (both shorthand and repo-relative classes). ~130 tokens now gated across the 8 must-be-true docs | W7 | — |
